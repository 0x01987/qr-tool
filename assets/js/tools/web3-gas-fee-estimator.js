document.addEventListener('DOMContentLoaded', () => {
  const NETWORKS = [
    ...
    { key: 'kcc', name: 'KCC', chainId: 321, symbol: 'KCS', rpc: 'https://rpc-mainnet.kcc.network' }
  ];
  ...
  async function estimateFeesEip1559(rpcUrl) {
    ...
    return {
      ...
      tiers: {
        slow: {
          maxFee: nextBaseFee * 2n + tipSlow,
          tip: tipSlow
        },
        std: {
          maxFee: nextBaseFee * 2n + tipStd,
          tip: tipStd
        },
        fast: {
          maxFee: nextBaseFee * 2n + tipFast,
          tip: tipFast
        }
      }
    };
  ...
  async function estimateFeesLegacy(rpcUrl) {
    ...
    return {
      ...
      tiers: {
        ...
        std: {
          maxFee: (gasPrice * 12n) / 10n,
          tip: 0n
        },
        fast: {
          maxFee: (gasPrice * 15n) / 10n,
          tip: 0n
        }
      }
    };
  ...
  function setTier(prefix, tier, symbol) {
  ...
  function updateUI(feeData) {
    const net = getSelectedNetwork();

    setTier('slow', feeData.tiers.slow, net.symbol);
    setTier('std', feeData.tiers.std, net.symbol);
    setTier('fast', feeData.tiers.fast, net.symbol);
  ...
  function buildSummaryText() {
    ...
    return [
      ...
      `Slow: ${els.slowNative ? els.slowNative.textContent : '—'} | ${els.slowMax ? els.slowMax.textContent : '—'} | ${els.slowTip ? els.slowTip.textContent : '—'} | ${els.slowTotal ? els.slowTotal.textContent : '—'}`,
      `Standard: ${els.stdNative ? els.stdNative.textContent : '—'} | ${els.stdMax ? els.stdMax.textContent : '—'} | ${els.stdTip ? els.stdTip.textContent : '—'} | ${els.stdTotal ? els.stdTotal.textContent : '—'}`,
      `Fast: ${els.fastNative ? els.fastNative.textContent : '—'} | ${els.fastMax ? els.fastMax.textContent : '—'} | ${els.fastTip ? els.fastTip.textContent : '—'} | ${els.fastTotal ? els.fastTotal.textContent : '—'}`
    ].join('\n');
  }
  ...
  async function refresh(useCache = true) {
    ...
    try {
      const feeData = await estimateFees(rpcUrl);
      updateUI(feeData);
      cacheSet(feeData);
