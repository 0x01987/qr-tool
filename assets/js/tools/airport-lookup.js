(function () {
  ...
  function setMapAirport(airport) {
    initMap();
    ...
    mapMarker.bindPopup(`
      ...
      ${escapeHtml(airport.city)}, ${escapeHtml(airport.country)}<br>
      IATA: ${escapeHtml(airport.iata || '—')}<br>
      ICAO: ${escapeHtml(airport.icao || '—')}
    `).openPopup();

    mapMetaEl.innerHTML = `
      Showing <strong>${escapeHtml(airport.name)}</strong> in ${escapeHtml(airport.city)}, ${escapeHtml(airport.country)}.
      Coordinates: ${escapeHtml(airport.lat)}, ${escapeHtml(airport.lon)}.
    `;
  ...
  function resetMap() {
    ...
    map.setView([20, 0], 2);
    mapMetaEl.textContent = 'The map will update when you select an airport result.';
  }
  ...
  function renderAirports(items, message) {
    ...
    resultsEl.innerHTML = items.map((airport, index) => `
      <article class="resultItem">
        ...
        <div class="actionsRow">
          <button class="cta ghost showMapBtn" type="button" data-airport-index="${escapeHtml(String(index))}">
            Show on Map
          </button>
        </div>
      </article>
    ...
    if (items[0]) {
      setMapAirport(items[0]);
    }
  }

  function bindMapButtons(items) {
    const buttons = Array.from(document.querySelectorAll('.showMapBtn'));
    buttons.forEach((btn) => {
      btn.addEventListener('click', () => {
        const idx = Number(btn.dataset.airportIndex);
        const airport = items[idx];
        if (airport) {
  ...
  function showPopular() {
    const popular = AIRPORTS.slice(0, 10);
    renderAirports(popular, 'No airports available.');
  }
  ...
  function waitForLeaflet() {
    ...
    const timer = setInterval(() => {
      ...
      if (typeof L !== 'undefined') {
        ...
        clearInterval(timer);
        mapMetaEl.textContent = 'Unable to load the map right now. Check whether a browser extension or network setting is blocking map files.';
      }
    }, 150);
  }
